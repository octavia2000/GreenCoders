  ...
  return (
    <nav className="flex items-center justify-between px-6 md:px-16 lg:px-28 py-4 bg-white">
      ...
      <div className="hidden md:flex space-x-8 absolute left-1/2 transform -translate-x-1/2">
        <Link to="/explore" className="text-gray-700 hover:text-eco-600 transition-colors font-medium">
          Shop
        </Link>
        <Link to="/categories" className="text-gray-700 hover:text-eco-600 transition-colors font-medium">
          Categories
        </Link>
        <Link to="/about" className="text-gray-700 hover:text-eco-600 transition-colors font-medium">
          About Us
        </Link>
        <Link to="/contact" className="text-gray-700 hover:text-eco-600 transition-colors font-medium">
          Contact Us
        </Link>
      </div>
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="hidden md:flex items-center space-x-4">
          ...
          {isAuthenticated ? (
            <div className="flex items-center space-x-2">
              <span className="text-sm text-gray-700 font-medium">Hi, {user?.name || 'User'}</span>
              <button
                onClick={logout}
        ...
        {/* Mobile Menu Button */}
        <button
          className={`md:hidden p-2 rounded-md transition-colors ${isOpen ? 'border border-green-500' : 'hover:bg-gray-100'}`}
      ...
      {isOpen && (
        <div className="absolute top-16 left-0 w-full bg-white shadow-lg border-t flex flex-col items-center space-y-4 py-6 md:hidden z-50">
          <Link to="/explore" className="text-gray-700 hover:text-eco-600 transition-colors font-medium" onClick={() => setIsOpen(false)}>
            Shop
          </Link>
          <Link to="/categories" className="text-gray-700 hover:text-eco-600 transition-colors font-medium" onClick={() => setIsOpen(false)}>
            Categories
          </Link>
          <Link to="/about" className="text-gray-700 hover:text-eco-600 transition-colors font-medium" onClick={() => setIsOpen(false)}>
            About Us
          </Link>
          <Link to="/contact" className="text-gray-700 hover:text-eco-600 transition-colors font-medium" onClick={() => setIsOpen(false)}>
            Contact Us
          </Link>
          ...
          <div className="border-t border-gray-200 w-full pt-4 mt-4">
            {isAuthenticated ? (
              <div className="space-y-2 text-center">
                <span className="block text-sm text-gray-700 font-medium">Hi, {user?.name || 'User'}</span>
                <button
                  onClick={() => {
                    logout();
                    setIsOpen(false);
              ...
              <div className="space-y-2 text-center">
                <Link to="/auth/login" className="block text-gray-700 hover:text-green-600 font-medium" onClick={() => setIsOpen(false)}>
                  Login
                </Link>
                <Link to="/auth/register" className="block text-gray-700 hover:text-green-600 font-medium" onClick={() => setIsOpen(false)}>
                  Sign Up
                </Link>
              </div>
            )}
